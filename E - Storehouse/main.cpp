...
bool charEquality (const char *a, const char *b) {
...
void setAW (Storehouse &storehouse, int we, int re, int se, int pe) {
    ...
    for (int i = we; i < 128; ++i) {
        storehouse.warehouse[i].handyShelf.placesAmount = 0;
        for (int j = 0; j < storehouse.warehouse[i].racksAmount; ++j) {
            for (int k = 0; k < storehouse.warehouse[i].rack[j].shelvesAmount; ++k) {
                for (int l = 0; l < storehouse.warehouse[i].rack[j].shelf[k].placesAmount; ++l) {
                    ...
                    storehouse.eSum -= storehouse.warehouse[i].handyShelf.place[l].ware;
                    storehouse.warehouse[i].handyShelf.place[l].ware = 0;
                    storehouse.warehouse[i].handyShelf.place[l].label[0] = '\0';
                    storehouse.warehouse[i].handyShelf.place[l].label[1] = '\0';
                }
...
void setHW (Storehouse &storehouse, int w, int p) {
    int previousAmount = storehouse.warehouse[w].handyShelf.placesAmount;
    storehouse.warehouse[w].handyShelf.placesAmount = p;
    
    for (int i = previousAmount; i < p; ++i) {
        storehouse.warehouse[w].handyShelf.place[i].ware = 0;
        storehouse.warehouse[w].handyShelf.place[i].label[0] = '\0';
        storehouse.warehouse[w].handyShelf.place[i].label[1] = '\0';
    }
    
    for (int i = p; i < 128; ++i) {
        storehouse.eSum -= storehouse.warehouse[w].handyShelf.place[i].ware;
...
void fill (Storehouse &storehouse, int w, int r, int s, int p, unsigned short a) {
    ...
    for (int i = 0; i < storehouse.warehousesAmount; ++i) {
        ...
        storehouse.warehouse[i].handyShelf.placesAmount = p;
        ...
        for (int j = 0; j < r; ++j) {
            ...
            for (int k = 0; k < s; ++k) {
                ...
                storehouse.handyRack.shelf[k].placesAmount = p;
                ...
                for (int l = 0; l < p; ++l) {
                    ...
                    if (!k && !j) {
                        ...
                        storehouse.warehouse[i].handyShelf.place[l].ware = a;
                        storehouse.warehouse[i].handyShelf.place[l].label[0] = '\0';
                        storehouse.warehouse[i].handyShelf.place[l].label[1] = '\0';
                    }
                    if (!k && !j && !i) {
...
unsigned long long getE (Storehouse &storehouse) { return storehouse.eSum; }

unsigned long long getW (Storehouse &storehouse, int w) {
    ...
    for (int i = 0; i < storehouse.warehouse[w].handyShelf.placesAmount; ++i) {
        sum += storehouse.warehouse[w].handyShelf.place[i].ware;
    }
    return sum;
}

unsigned long long getRW (Storehouse &storehouse, int w, int r) {
    unsigned long long sum = 0;
...
unsigned long long getRH (Storehouse &storehouse) {
    unsigned long long sum = 0;
    for (int i = 0; i < storehouse.handyRack.shelvesAmount; ++i) {
        for (int j = 0; j < storehouse.handyRack.shelf[i].placesAmount; ++j) {
            sum += storehouse.handyRack.shelf[i].place[j].ware;
        }
    }
    return sum;
}

unsigned long long getSW (Storehouse &storehouse, int w, int r, int s) {
    unsigned long long sum = 0;
...
unsigned long long getSH (Storehouse &storehouse, int w) {
    unsigned long long sum = 0;
    for (int i = 0; i < storehouse.warehouse[w].handyShelf.placesAmount; ++i)
        sum += storehouse.warehouse[w].handyShelf.place[i].ware;
    return sum;
...
unsigned long long getSR (Storehouse &storehouse, int s) {
    unsigned long long sum = 0;
...
unsigned long long getS (Storehouse &storehouse) {
    unsigned long long sum = 0;
    for (int i = 0; i < storehouse.handyShelf.placesAmount; i++)
        sum += storehouse.handyShelf.place[i].ware;
    return sum;
...
void putH (Storehouse &storehouse, int w, int p, int a) {
    if ((storehouse.warehouse[w].handyShelf.place[p].ware + a) >= 65535) {
        storehouse.eSum += 65535 - storehouse.warehouse[w].handyShelf.place[p].ware;
        storehouse.warehouse[w].handyShelf.place[p].ware = 65535;
    }
    else {
        storehouse.eSum += a;
        storehouse.warehouse[w].handyShelf.place[p].ware += a;
    }
}
...
void popH (Storehouse &storehouse, int w, int p, int a) {
    if ((storehouse.warehouse[w].handyShelf.place[p].ware - a) <= 0) {
        storehouse.eSum -= storehouse.warehouse[w].handyShelf.place[p].ware;
        storehouse.warehouse[w].handyShelf.place[p].ware = 0;
    }
    else {
        storehouse.eSum -= a;
        storehouse.warehouse[w].handyShelf.place[p].ware -= a;
...
void setLH (Storehouse &storehouse, int w, int p, unsigned char dd[2]) {
    storehouse.warehouse[w].handyShelf.place[p].label[0] = dd[0];
    storehouse.warehouse[w].handyShelf.place[p].label[1] = dd[1];
}
...
void getLW (Storehouse &storehouse, int w, int r, int s, int p) {
    if (storehouse.warehouse[w].rack[r].shelf[s].place[p].label[0] == '\0' &&
        storehouse.warehouse[w].rack[r].shelf[s].place[p].label[1] == '\0') cout << "--" << endl;
    else cout << storehouse.warehouse[w].rack[r].shelf[s].place[p].label[0] << storehouse.warehouse[w].rack[r].shelf[s].place[p].label[1] << endl;
}

void getLH (Storehouse &storehouse, int w, int p) {
    if (storehouse.warehouse[w].handyShelf.place[p].label[0] == '\0' &&
        storehouse.warehouse[w].handyShelf.place[p].label[1] == '\0') cout << "--" << endl;
    else cout << storehouse.warehouse[w].handyShelf.place[p].label[0] << storehouse.warehouse[w].handyShelf.place[p].label[1] << endl;
}

void getLR (Storehouse &storehouse, int s, int p) {
    if (storehouse.handyRack.shelf[s].place[p].label[0] == '\0' && storehouse.handyRack.shelf[s].place[p].label[1] == '\0') cout << "--" << endl;
    else cout << storehouse.handyRack.shelf[s].place[p].label[0] << storehouse.handyRack.shelf[s].place[p].label[1] << endl;
}

void getLS (Storehouse &storehouse, int p) {
    if (storehouse.handyShelf.place[p].label[0] == '\0' && storehouse.handyShelf.place[p].label[1] == '\0') cout << "--" << endl;
    else cout << storehouse.handyShelf.place[p].label[0] << storehouse.handyShelf.place[p].label[1] << endl;
}
...
void movW (Storehouse &storehouse, int wb, int rb, int sb, int we, int re, int se, int p, int a) {
    ...
    if (storehouse.warehouse[wb].rack[rb].shelf[sb].place[p].ware < a)
...
void movH (Storehouse &storehouse, int w, int r, int s, int p, int a) {
    if ((storehouse.warehouse[w].handyShelf.place[p].ware + a) > 65535)
        a = 65535 - storehouse.warehouse[w].handyShelf.place[p].ware;
    if (storehouse.warehouse[w].rack[r].shelf[s].place[p].ware < a)
...
void movR (Storehouse &storehouse, int w, int r, int sb, int se, int p, int a) {
    ...
    if (storehouse.warehouse[w].rack[r].shelf[sb].place[p].ware < a)
        a = storehouse.warehouse[w].rack[r].shelf[sb].place[p].ware;
    storehouse.handyRack.shelf[se].place[p].ware += a;
    storehouse.warehouse[w].rack[r].shelf[sb].place[p].ware -= a;
}

void movS (Storehouse &storehouse, int s, int p, int a) {
    if ((storehouse.handyShelf.place[p].ware + a) > 65535)
        a = 65535 - storehouse.handyShelf.place[p].ware;
    if (storehouse.handyRack.shelf[s].place[p].ware < a)
...
int main() {
    ...
    do {
        ...
        else if (charEquality(order, "SET-AS")) {
            int wb, rb, se, pe;
            cin >> wb >> rb >> se >> pe;
        ...
        //SET-L
        else if (charEquality(order, "SET-LW")) {
        ...
        else if (charEquality(order, "SET-LH")) {
            int w, p; cin >> w >> p;
            unsigned char dd[2]; cin >> dd[0] >> dd[1];
            if (w >= storehouse.warehousesAmount || p >= storehouse.warehouse[w].handyShelf.placesAmount) cout << "error\n";
            else setLH(storehouse, w, p, dd);
        }
        else if (charEquality(order, "SET-LR")) {
            int s, p; cin >> s >> p;
            unsigned char dd[2]; cin >> dd[0] >> dd[1];
        ...
        else if (charEquality(order, "SET-LS")) {
            int p; cin >> p;
            unsigned char dd[2]; cin >> dd[0] >> dd[1];
        ...
        // FILL
        else if (charEquality(order, "FILL")) {
            int w, r, s, p, a;
            cin >> w >> r >> s >> p >> a;
            if (a > 65535) a = 65535;
            if (w > 128 || r > 128 || s > 128 || p > 128) cout << "error\n";
            else fill(storehouse, w, r, s, p, a);
        }
        
        // GET
        else if (charEquality(order, "GET-E")) {
            cout << getE(storehouse) << "\n";
        }
        else if (charEquality(order, "GET-W")) {
            int w; cin >> w;
            if (w >= storehouse.warehousesAmount) cout << "error\n";
            else cout << getW(storehouse, w) << "\n";
        }
        else if (charEquality(order, "GET-RW")) {
            int w, r; cin >> w >> r;
        ...
        else if (charEquality(order, "GET-RH")) {
            cout << getRH(storehouse) << "\n";
        }
        else if (charEquality(order, "GET-SW")) {
            int w, r, s; cin >> w >> r >> s;
        ...
        else if (charEquality(order, "GET-SH")) {
            int w; cin >> w;
            
            if (w >= storehouse.warehousesAmount) cout << "error\n";
            else cout << getSH(storehouse, w) << "\n";
        }
        else if (charEquality(order, "GET-SR")) {
            int s; cin >> s;
            
            if (s >= storehouse.handyRack.shelvesAmount) cout << "error\n";
            else cout << getSR(storehouse, s) << "\n";
        }
        else if (charEquality(order, "GET-S")) {
            cout << getS(storehouse) << "\n";
        }
        
        
        // GET-L
        else if (charEquality(order, "GET-LW")) {
            int w, r, s, p; cin >> w >> r >> s >> p;
            if (w >= storehouse.warehousesAmount || r >= storehouse.warehouse[w].racksAmount || s >= storehouse.warehouse[w].rack[r].shelvesAmount || p >= storehouse.warehouse[w].rack[r].shelf[s].placesAmount) cout << "error\n";
            else getLW(storehouse, w, r, s, p);
        }
        else if (charEquality(order, "GET-LH")) {
            int w, p; cin >> w >> p;
            if (w >= storehouse.warehousesAmount || p >= storehouse.warehouse[w].handyShelf.placesAmount) cout << "error\n";
            else getLH(storehouse, w, p);
        }
        else if (charEquality(order, "GET-LR")) {
            int s, p; cin >> s >> p;
            if (s >= storehouse.handyRack.shelvesAmount || p >= storehouse.handyRack.shelf[s].placesAmount) cout << "error\n";
            else getLR(storehouse, s, p);
        }
        else if (charEquality(order, "GET-LS")) {
            int p; cin >> p;
            if (p >= storehouse.handyShelf.placesAmount) cout << "error\n";
        ...
        // PUT
        else if (charEquality(order, "PUT-W")) {
            int w, r, s, p, a; cin >> w >> r >> s >> p >> a;
            if (w >= storehouse.warehousesAmount || r >= storehouse.warehouse[w].racksAmount || s >= storehouse.warehouse[w].rack[r].shelvesAmount || p >= storehouse.warehouse[w].rack[r].shelf[s].placesAmount) cout << "error\n";
            else putW(storehouse, w, r, s, p, a);
        }
        else if (charEquality(order, "PUT-H")) {
            int w, p, a; cin >> w >> p >> a;
            if (w >= storehouse.warehousesAmount || p >= storehouse.warehouse[w].handyShelf.placesAmount) cout << "error\n";
            else putH(storehouse, w, p, a);
        }
        else if (charEquality(order, "PUT-R")) {
            int s, p, a; cin >> s >> p >> a;
            if (s >= storehouse.handyRack.shelvesAmount || p >= storehouse.handyRack.shelf[s].placesAmount) cout << "error\n";
            else putR(storehouse, s, p, a);
        }
        else if (charEquality(order, "PUT-S")) {
            int p, a; cin >> p >> a;
            if (p >= storehouse.handyShelf.placesAmount) cout << "error\n";
        ...
        // POP
        else if (charEquality(order, "POP-W")) {
            int w, r, s, p, a; cin >> w >> r >> s >> p >> a;
            if (w >= storehouse.warehousesAmount || r >= storehouse.warehouse[w].racksAmount || s >= storehouse.warehouse[w].rack[r].shelvesAmount || p >= storehouse.warehouse[w].rack[r].shelf[s].placesAmount) cout << "error\n";
            else popW(storehouse, w, r, s, p, a);
        }
        else if (charEquality(order, "POP-H")) {
            int w, p, a; cin >> w >> p >> a;
            if (w >= storehouse.warehousesAmount || p >= storehouse.warehouse[w].handyShelf.placesAmount) cout << "error\n";
            else popH(storehouse, w, p, a);
        }
        else if (charEquality(order, "POP-R")) {
            int s, p, a; cin >> s >> p >> a;
            if (s >= storehouse.handyRack.shelvesAmount || p >= storehouse.handyRack.shelf[s].placesAmount) cout << "error\n";
            else popR(storehouse, s, p, a);
        }
        else if (charEquality(order, "POP-S")) {
            int p, a; cin >> p >> a;
            if (p >= storehouse.handyShelf.placesAmount) cout << "error\n";
        ...
        // MOV
        else if (charEquality(order, "MOV-W")) {
        ...
        else if (charEquality(order, "MOV-H")) {
            int w, r, s, p, a;
            cin >> w >> r >> s >> p >> a;
            if (w >= storehouse.warehousesAmount) cout << "error\n";
            ...
            else if (p >= storehouse.warehouse[w].handyShelf.placesAmount) cout << "error\n";
            else movH(storehouse, w, r, s, p, a);
        }
        else if (charEquality(order, "MOV-R")) {
            int w, r, sb, se, p, a;
            cin >> w >> r >> sb >> se >> p >> a;
            if (w >= storehouse.warehousesAmount) cout << "error\n";
        ...
        else if (charEquality(order, "MOV-S")) {
            int s, p, a;
            cin >> s >> p >> a;
            if (s >= storehouse.handyRack.shelvesAmount) cout << "error\n";
